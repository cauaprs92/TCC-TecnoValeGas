...
function gerarProdutosObraGrande() {
  const idObra = document.getElementById('obraIdEdicao').value;
  if (!idObra) {
    showToast('Salve a obra antes de gerar o documento.', 'warning');
    return;
  }
  window.open(`/produtos-obra-grande.html?id=${idObra}`, '_blank');
}
...
async function renderGraficoProdutos(dias = '') {
  ...
  if (!document.getElementById('produtosChartCanvas')) {
    container.innerHTML = '';
    const c = document.createElement('canvas');
    c.id = 'produtosChartCanvas';
    container.appendChild(c);
  }
  const cvs = document.getElementById('produtosChartCanvas');
  ...
  _produtosChart = new Chart(cvs.getContext('2d'), {
    type: 'bar',
    data: {
